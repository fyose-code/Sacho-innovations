...
const adminLinks = [
  ...
  { name: "Task Manager", path: "/admin/tasks", icon: CheckSquare },
  { name: "Team & Attendance", path: "/admin/team", icon: Shield },
  { name: "Team Profiles", path: "/admin/team-profiles", icon: User },
  { name: "Communications", path: "/admin/messages", icon: MessageSquare },
  { name: "Conference Room", path: "/admin/conference", icon: Globe },
  { name: "Financials", path: "/admin/billing", icon: CreditCard },
];
    ...
    <div className="min-h-screen bg-brand-bg text-[#1a1a1a] font-sans flex overflow-hidden">
      ...
      >
          ...
          <Link to="/" className="flex items-center gap-2 overflow-hidden">
            ...
            {isSidebarOpen && (
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-brand-yellow font-medium tracking-[0.2em] uppercase">Admin</span>
              </div>
            )}
          </Link>
        </div>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
          {adminLinks.map((link) => (
            ...
            >
              <link.icon className={cn("w-5 h-5", location.pathname === link.path ? "text-white" : "text-gray-500 group-hover:text-brand-blue")} />
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="h-20 bg-white/40 backdrop-blur-xl border-b border-black/5 px-6 flex items-center justify-between z-40">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-brand-blue/10 border border-brand-blue/20 rounded-full">
              <Zap className="w-3 h-3 text-brand-blue" />
              <span className="text-[10px] font-bold uppercase tracking-widest text-brand-blue">Admin Mode</span>
            </div>
                  ...
                  <>
                    ...
                    >
                      <div className="p-4 border-b border-black/5 flex items-center justify-between">
                        <h3 className="font-bold text-sm text-[#1a1a1a]">Admin Alerts</h3>
                        {unreadCount > 0 && (
                          <button 
                        ...
                        {notifications.length > 0 ? (
                          notifications.map((notif) => (
                            ...
                            >
                              ...
                              <p className="text-[8px] text-gray-500 mt-2 uppercase tracking-widest font-bold">
          ...
          <>
            ...
            >
              <div className="p-6 flex items-center justify-between">
                ...
                <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-gray-500 hover:text-brand-blue transition-colors">
                  <X />
                </button>
              </div>
              <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
